...
public class Main3055_탈출_백준 {
    ...
    private static void input() throws IOException {
		...
		for (int i = 0; i < r; i++) {
			...
			for (int j = 0; j < c; j++) {
				switch (line[j]) {
				case 'S':
					map[i][j] = 0; start = new int[]{i, j}; break;
				case 'D':
					map[i][j] = 0; cave = new int[]{i, j}; break;
				case '.':
					map[i][j] = 0; break;
				case 'X':
					map[i][j] = 1; break;
				case '*':
					map[i][j] = 2; floods.add(new Flood(i, j)); break;
				}
			}
		}
	}
    ...
    private static void escape() {
    	...
    	for (Flood flood : floods) que.offer(flood);
    ...
    private static void print() {
    	...
    	if (alive) System.out.println(minTime);
